...
export class PreferencesStore {
  ...
  private readonly mockStore: Map<string, Record<string, Record<string, unknown>>>
  private dynamoClient?: DynamoDBDocumentClient

  constructor(config: PreferencesStoreConfig = {}) {
    this.config = {
      tableName: config.tableName ?? process.env['DYNAMODB_TABLE_NAME'] ?? 'yuppee-preferences',
  ...
  async getPreferences(userId: string): Promise<Record<string, Record<string, unknown>>> {
    if (this.config.useMock) {
  ...
  async savePreferences(userId: string, queryCategory: string, prefs: Record<string, unknown>): Promise<void> {
    if (this.config.useMock) {
  ...
  private async realGetPreferences(userId: string): Promise<Record<string, Record<string, unknown>>> {
    ...
    const { preferences } = result.Item as { preferences?: Record<string, Record<string, unknown>> }
    return preferences ?? {}
  }
